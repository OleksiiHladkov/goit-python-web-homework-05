...
class Logger:
    ...
    async def logging_exchange_command(self, response: list | str, message: str) -> None:
        rfn = resource_filename("web_socket_chat", self.path)
        async with async_open(rfn, "a+") as afp:
            format = "%d/%m/%Y, %H:%M:%S"
            await afp.write(
                f"{self.time.strftime(format)} - user: {self.user.name} - command: [{self.data}] - response: {str(response)} - message: [{message}]\n"
            )
...
class Server:
    ...
    async def send_to_client(self, message: str, ws: WebSocketServerProtocol) -> None:
        if self.clients:
    ...
    async def exchange_data_handler(self, exchange_data: list[dict] | str) -> str:
